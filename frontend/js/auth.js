...
async function handleGoogleCallback(response) {
  ...
  try {
    ...
    // Any authenticated Google/Workspace account gets access in restricted mode
    // Optionally verify against allow-list (if configured)
    try {
      ...
      // If allowlist is configured and email is NOT in it, deny
      // If allowlist is empty/not configured, allow any Google account
      if (data.restricted && !data.allowed) {
        denyAccess(user);
      } else {
        grantAccess(user);
      }
    } catch {
      // If verify endpoint fails, grant access to any authenticated Google user
      grantAccess(user);
    }
  } catch (err) {
...
async function bootAuth() {
  // Step 1: Check public access mode from server
  ...
  // Step 2: If open access, grant directly without login
  if (isOpenAccess) {
    // Show app without requiring login
    ...
    window.currentUser = loadSession() || { email: 'public', name: 'Visitante', mode: 'open' };
    if (typeof window.initApp === 'function') window.initApp();
    return;
  }

  // Step 3: Restricted mode — check session
